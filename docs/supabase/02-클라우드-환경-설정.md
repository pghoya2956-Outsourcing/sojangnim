...
프로젝트가 생성되면 **Settings** 메뉴에서 다음 정보를 확인할 수 있습니다:

| 정보 | 메뉴 위치 | 환경 변수 이름 |
|------|----------|---------------|
| Project URL | Settings → **Data API** | `NEXT_PUBLIC_SUPABASE_URL` |
| Publishable key | Settings → **API Keys** | `NEXT_PUBLIC_SUPABASE_ANON_KEY` |
| Secret key | Settings → **API Keys** | 서버 전용 (노출 금지) |

```
Project URL: https://abcdefgh.supabase.co
Publishable key: sb_publishable_xxx...  → NEXT_PUBLIC_SUPABASE_ANON_KEY
Secret key: sb_secret_xxx...            → 서버 전용 (Vercel에 넣지 않음)
```
...
> **💡 프로젝트 1개만 사용한다면?**
> 3~7단계에서 만든 프로젝트를 그대로 프로덕션으로 사용하면 됩니다.
> **이 단계는 건너뛰세요!**

dev/prod 분리 전략을 사용하는 경우에만 진행합니다.
...
그 다음 **6단계**(초기 데이터 입력), **7단계**(Admin 사용자 추가)를 프로덕션 프로젝트에도 동일하게 진행합니다.
